        ...
        <form wire:submit.prevent="update" method="POST">
            ...
            <div class="grid mt-2 gap-5 lg:grid-cols-2 sm:max-w-sm sm:mx-auto lg:max-w-full">
                <div class="mt-4">
                    ...
                    <span class="text-sm mb-4">{{ __('Example: MyShop.tnyweb.com') }}</span>
                </div>
                <div class="mt-4">
                    <div class="text-gray-700 dark:text-gray-400 font-medium">
                        <input wire:model="domain"
...
{{--                                //todo change this --}}
                                <br>75.2.70.75 - is our Websiter Server IP address
            ...
            <button form="cancel" class="mt-2 px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-gray-400 border border-transparent rounded-lg active:bg-gray-600 hover:bg-gray-600 focus:outline-none focus:shadow-outline-gray">
                {{ __('Cancel') }}
            </button>
        </form>
        <form id="cancel" method="GET" action="{{ route('project.index') }}"></form>
    </div>
</div>
